...
(
    ...
    WHERE
        PROPERTY_TYPE_CODE=4 AND KO_UNIT.TASK_ID IN ({2},{3})
)
SELECT
    Zone,
    ZoneNameByCircles,
    ...
    SUM(ObjectUpks) FILTER (WHERE TourId={4}) AS FirstTourSumUpks,
    ...
    SUM(ObjectUpks) FILTER (WHERE TourId={5}) AS SecondTourSumUpks,
