(function ($) {
    ...
    FilterQueryBuilder.prototype.init = function (config) {
        ...
        function initControls() {
            self.$element.find('.alert.alert-warning.alert-group:first').prepend('<div style="margin-bottom: 5px;"><label style="width: 60px;">Запрос:</label><input id="f_txt_query" type="text" class="k-textbox" style="width: 84%;"><button class="k-button f_button_save" style="margin-left: 5px;" title="Сохранить фильтр" data-bind="saveFiltr"><span class="k-icon k-i-save"></span>&nbsp;</button><button class="k-button f_button_delete" style="margin-left: 5px;" title="Удалить фильтр" data-bind="deleteFiltr"><span class="k-icon k-i-delete"></span>&nbsp;</button></div>');
        }
        ...
        function fillStruct() {
            ...
            if (struct) {
                if (struct.group && struct.group.length > 0) {
                    var group = addGroup(self.$element, { operator: struct.group[0].operator }, true)
        ...
        function addCondition(group, params) {
            if (group) {
                ...
                var ddTree = $res.find('input.s_attributes:first').kendoDropDownTree({
                    ...
                    change: function (e) {
                        if (this.value() != 0) {
                            setValidation(e.sender.element.closest('.condition'));
                            setQuery(self.$element);
                        }
                    }
                });
