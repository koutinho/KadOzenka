...
namespace KadOzenka.Dal.GbuObject
{
    public class GbuObjectService : IGbuObjectService
    {
        ...
        public static GbuObjectAttribute CheckExistsValueFromAttributeIdPartition(long objectId, long attributeId,
            DateTime otDate)
        {
            ...
            var sql =
                $@"select A.change_date as ChangeDate, A.change_doc_id as ChangeDocId from {registerData.AllpriTable}_{attributeData.Id} A where A.object_Id = {objectId} and A.Ot = {CrossDBSQL.ToDate(otDate)}";

            _log.ForContext("SQL", sql)
                .ForContext("OtDate", CrossDBSQL.ToDate(otDate))
                .Verbose("Проверка наличия значения по объекту {ObjectId} в {AllpriTable}_{AttributeDataId}", objectId,
                    registerData.AllpriTable, attributeData.Id);
                ...
                foreach (var registerId in uniqueSources)
                {
                    ...
                    if (registerData.AllpriPartitioning == Platform.Register.AllpriPartitioningType.DataType)
                    {
                        ...
                        foreach (var postfix in postfixes)
                        {
                            ...
                            result.AddRange(_queryManager != null
                                ? _queryManager.ExecuteSql<GbuObjectAttribute>(sql)
                                : QSQuery.ExecuteSql<GbuObjectAttribute>(sql));
                        }
                    }
        ...
        private static string GetSqlForRegisterWithDataPartitioning(List<long> objectIds, string postfix,
            string propName, RegisterData registerData, List<GbuColumnsToDownload> attributesToDownload)
        {
        ...
        private static string GetSqlForRegisterWithAttributePartitioning(List<long> objectIds, string propName,
            RegisterAttribute attributeData, RegisterData registerData,
            List<GbuColumnsToDownload> attributesToDownload = null)
        {
            if (attributesToDownload != null)
            {
        ...
        public static string GetSqlForColumns(List<GbuColumnsToDownload> attributes, string valueColumnAlias,
            string attributeIdColumnName)
        {
        ...
        public List<AllDataTreeDto> GetAllDataTree(long objectId, string parentNodeId, long nodeLevel)
        {
            ...
            if (parentNodeId.IsNullOrEmpty())
            {
                ...
                foreach (var register in RegisterCache.Registers.Values
                    .Where(x => x.QuantTable == mainRegisterData.QuantTable && x.Id != mainRegisterData.Id)
                    .OrderBy(x => x.Description))
                {
                    ...
                    {
                        foreach (var attributeData in RegisterCache.RegisterAttributes.Values.Where(x =>
                            x.RegisterId == register.Id && x.IsPrimaryKey != true))
                        {
                            string sql =
                                $"select count(1) as ValuesCount from {register.AllpriTable}_{attributeData.Id} a where a.object_id = {objectId}";
                    ...
                    else
                    {
                        ...
                        {
                            string sql =
                                $"select count(1) as ValuesCount from {register.AllpriTable}_{postfix} a where a.object_id = {objectId}";
                    ...
                    result.Add(new AllDataTreeDto
                    {
                        ...
                        Level = 0,
                        ContentUrl = $"/GbuObject/AllDetails?objectId={objectId}&registerId={register.Id}"
                            .ResolveClientUrl(),
                        HasChilds = true
                    });
                }
            ...
            else
            {
                ...
                {
                    foreach (var attributeData in RegisterCache.RegisterAttributes.Values.Where(x =>
                        x.RegisterId == parentRegisterData.Id && x.IsPrimaryKey != true))
                    {
                        string sql =
                            $"select {attributeData.Id} as AttributeId, count(1) as ValuesCount from {parentRegisterData.AllpriTable}_{attributeData.Id} a where a.object_id = {objectId}";
                ...
                else
                {
                    ...
                    {
                        string sql =
                            $"select a.attribute_id as AttributeId, count(1) as ValuesCount from {parentRegisterData.AllpriTable}_{postfix} a where a.object_id = {objectId} group by a.attribute_id";
                ...
                foreach (var attributeValueCount in attributesValuesCount)
                {
                    result.Add(new AllDataTreeDto
                    {
                        ...
                        NodeText =
                            $"{RegisterCache.GetAttributeData((int) attributeValueCount.AttributeId).Name} ({attributeValueCount.ValuesCount})",
                        Level = 1,
                        ContentUrl =
                            $"/GbuObject/AllDetails?objectId={objectId}&attributeId={attributeValueCount.AttributeId}"
                                .ResolveClientUrl(),
                        HasChilds = false
                    });
        ...
        public List<GbuAttributeValueObjectsCountDto> GetAttributeValueKoObjectsCount(long attributeId,
            KoUnitStatus koUnitStatus, DateTime? taskCreationDateFrom, DateTime? taskCreationDateTo,
        ...
        {
            return OMAttribute
                .Where(x => GetGbuRegistersIds().Contains(x.RegisterId) && x.IsDeleted.Coalesce(false) == false)
                .SelectAll().Select(x => x.ParentRegister.RegisterDescription).Execute();
        }
        ...
        {
            if (CheckExistsValueFromAttributeIdPartition(attributeValue.ObjectId, attributeValue.AttributeId,
                attributeValue.Ot) != null)
            {
                // Проблема в наличии значения на ту же дату ОТ
                attributeValue.Ot = GetNextOtFromAttributeIdPartition(attributeValue.ObjectId,
                    attributeValue.AttributeId, attributeValue.Ot);
            }
