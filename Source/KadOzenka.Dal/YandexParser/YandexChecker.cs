...
namespace KadOzenka.Dal.YandexParser
{
	public class YandexChecker
	{
		...
		public YandexChecker()
		{
			FormMarketObjectsRequests = new List<FormMarketObjectsRequest>();
		}
		...
		public void FormMarketObjects()
		{
			...
			//TODO: добавить FormMarketObjectsRequests'ты для остальных категорий
					...
					try
					{
						...
						var pagerButton =
							((ChromeDriver)driver).ExecuteScript(
								ConfigurationManager.AppSettings["getYandexPagerNextButton"]) as IWebElement;
						var nextPage = 1;
						...
						foreach (var objectUrl in uniqueObjectsUrls)
						{
							try
							{
								...
								((IJavaScriptExecutor) driver)
									.ExecuteScript(
										File.ReadAllText(
											ConfigurationManager.AppSettings["YandexGetInitialStateJsPath"]),
										objectUrl);
								var val = new WebDriverWait(driver, TimeSpan.FromSeconds(5))
								...
								var marketObject = FillMarketObject(formMarketObjectsRequest, obj, objectUrl);
								createdObjects.Add(marketObject);

								KKad++;
							}
							catch (Exception e)
							{
								Console.WriteLine($"Произошла ошибка во время парсинга объекта ({objectUrl}):{e.Message}. Запущен повторный парсинг");
								try
								{
									driver.Navigate().GoToUrl(objectUrl);
									CheckCapcha((ChromeDriver)driver);
									...
									Console.WriteLine($"Объект {objectUrl} был успешно обработан");
									KKad++;
								}
								catch (Exception ex)
								{
									KErr++;
									Console.WriteLine(
										$"Не удалось распарсить объект ({objectUrl}):{ex.Message}");
								}
							}
						...
						Console.WriteLine(
							$"Парсинг для категории {formMarketObjectsRequest.ObjectsListUrl} завершен. Успешно добавлено {createdObjects.Count} объектов.");
					}
					catch (Exception e)
					{
						Console.WriteLine(
		...
		{
			var isCapcha = driver.ExecuteScript(
				ConfigurationManager.AppSettings["isYandexCapchaScreen"]).ToString();
		...
		{
			var val = (ReadOnlyCollection<object>)javaScriptExecutor.ExecuteScript(ConfigurationManager.AppSettings["getYandexUrlList"]);
			return val.Select(x => (string)x).ToList();
		}
		...
		private OMCoreObject FillMarketObject(FormMarketObjectsRequest formMarketObjectsRequest, JObject obj, string objectUrl)
		{
			...
			marketObject.Price = obj.SelectToken("cards.offers.price").HasValues
				? obj.SelectToken("cards.offers.price.value")?.Value<long>()
				: (long?)null;
			marketObject.ParserTime = DateTime.Now;
			marketObject.Region = null;
			marketObject.City = "Москва";

			marketObject.Address = obj.SelectToken("cards.offers.location").HasValues
				? obj.SelectToken("cards.offers.location.address")?.Value<string>()
				: null;
			if (obj.SelectToken("cards.offers.location.metroList").HasValues)
			{
				var metroList = new List<string>();
				for (var i = 0; i < obj.SelectToken("cards.offers.location.metroList").Count(); i++)
				{
					metroList.Add(obj.SelectToken($"cards.offers.location.metroList[{i}].name")?.Value<string>());
				}
			...
			if (obj.SelectToken("cards.offers.fullImages").HasValues)
			{
				...
				for (var i = 0; i < obj.SelectToken("cards.offers.fullImages").Count(); i++)
				{
					imagesList.Add(obj.SelectToken($"cards.offers.fullImages[{i}]")?.Value<string>());
				}
			...
			marketObject.Lat = obj.SelectToken("cards.offers.location.point").HasValues
				? obj.SelectToken("cards.offers.location.point.latitude")?.Value<decimal>()
				: (decimal?)null;
			marketObject.Lng = obj.SelectToken("cards.offers.location.point")
				.HasValues
				? obj.SelectToken("cards.offers.location.point.longitude")?.Value<decimal>()
				: (decimal?)null;
			...
			if (formMarketObjectsRequest.PropertyType != PropertyTypes.Stead)
			{
				marketObject.Area = GetObjectArea(obj);
			}
			...
			if (wallMaterial.HasValue)
			{
				marketObject.WallMaterial_Code = wallMaterial.Value;
			}
