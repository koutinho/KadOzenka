...
{
	public class ResultsByCadastralDistrictForPlacementsReportLongProcess : ALinearReportsLongProcessTemplate<ResultsByCadastralDistrictForPlacementsReportLongProcess.ReportItem, InputParametersForPlacements>
	{
		protected override string ReportName => "Результаты в разрезе КР (Помещения)";
		protected override string ProcessName => nameof(ResultsByCadastralDistrictForPlacementsReportLongProcess);
		...
		protected override long ReportCode => (long)StatisticalDataType.ResultsByKRForPlacements;



		public ResultsByCadastralDistrictForPlacementsReportLongProcess() : base(Log.ForContext<ResultsByCadastralDistrictForPlacementsReportLongProcess>())
		{
			RosreestrRegisterService = new RosreestrRegisterService();
		}


		protected override bool AreInputParametersValid(InputParametersForPlacements inputParameters)
		{
		...
		protected override void PrepareVariables(InputParametersForPlacements inputParameters)
		{
		...
		protected override ReportsConfig GetProcessConfig()
		{
			...
			return GetProcessConfigFromSettings("ResultsByCadastralDistrictForPlacements", defaultPackageSize, defaultThreadsCount);
		}

		protected override int GetMaxItemsCount(InputParametersForPlacements inputParameters)
		{
		...
		protected override List<Column> GenerateReportHeaders()
		{
			var columns = new List<Column>
			{
				...
				new Column {Header = "Назначение здания или сооружения, в котором расположено помещение", Width = 3},
				new Column {Header = "Кадастровый номер здания или сооружения, в котором расположено помещение", Width = ColumnWidthForCadastralNumber},
				new Column {Header = "Номер подгруппы здания или сооружения, в котором расположено помещение", Width = 3},
		...
		private string GetBaseSql(InputParametersForPlacements inputParameters)
		{
			var tourId = GetTourFromTasks(inputParameters.TaskIds);

			var baseFolderWithSql = "ResultsByCadastralDistrict";
			var sql = StatisticalDataService.GetSqlFileContent(baseFolderWithSql, "PlacementsForLongProcess");
