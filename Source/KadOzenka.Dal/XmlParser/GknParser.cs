...
namespace KadOzenka.Dal.XmlParser
{
    public class xmlImportGkn
    {
        ...
        public xmlObject GetData(XmlNode xnObjectNode, enTypeObject typeobject, DateTime assessmentDate)
        {
            ...
            try
            {
                kn = (xnObjectNode.Attributes["CadastralNumber"] == null)
                    ? string.Empty
                    : xnObjectNode.Attributes["CadastralNumber"].InnerText;
                DateTime dc = (xnObjectNode.Attributes["DateCreated"] == null)
                    ? DateTime.MinValue
                    : Convert.ToDateTime(xnObjectNode.Attributes["DateCreated"].InnerText);
                ...
                foreach (XmlNode xnChild in xnObjectNode.ChildNodes)
                {
                    switch (xnChild.Name)
                    {
                        ...
                        case "CadastralCost":
                            ...
                            if (xnChild.Attributes["Value"] != null)
                                obj.CadastralCost.Value = xnChild.Attributes["Value"].InnerText.ParseToDouble();
                            foreach (XmlNode xnChild1 in xnChild.ChildNodes)
                            {
                                if (xnChild1.Name == "DateValuation")
                                {
                                    obj.CadastralCost.DateValuation =
                                        DateTime.TryParse(xnChild1.InnerText, out var date)
                                            ? date
                                            : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "DateEntering")
                                {
                                    obj.CadastralCost.DateEntering = DateTime.TryParse(xnChild1.InnerText, out var date)
                                        ? date
                                        : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "DateApproval")
                                {
                                    obj.CadastralCost.DateApproval = DateTime.TryParse(xnChild1.InnerText, out var date)
                                        ? date
                                        : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "DocNumber")
                                {
                                    obj.CadastralCost.DocNumber = xnChild1.InnerText;
                                }
                                else if (xnChild1.Name == "DocDate")
                                {
                                    obj.CadastralCost.DocDate = DateTime.TryParse(xnChild1.InnerText, out var date)
                                        ? date
                                        : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "ApplicationDate")
                                {
                                    obj.CadastralCost.ApplicationDate =
                                        DateTime.TryParse(xnChild1.InnerText, out var date)
                                            ? date
                                            : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "RevisalStatementDate")
                                {
                                    obj.CadastralCost.RevisalStatementDate =
                                        DateTime.TryParse(xnChild1.InnerText, out var date)
                                            ? date
                                            : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "ApplicationLastDate")
                                {
                                    obj.CadastralCost.ApplicationLastDate =
                                        DateTime.TryParse(xnChild1.InnerText, out var date)
                                            ? date
                                            : (DateTime?) null;
                                }
                                else if (xnChild1.Name == "DocName")
                                {
                        ...
                        case "Floors":
                            ...
                            if (xnChild.Attributes["Floors"] != null)
                                obj.Floors.Floors = xnChild.Attributes["Floors"].InnerText;
                            if (xnChild.Attributes["UndergroundFloors"] != null)
                                obj.Floors.Underground_Floors = xnChild.Attributes["UndergroundFloors"].InnerText;
                        ...
                        case "ExploitationChar":
                            ...
                            if (xnChild.Attributes["YearBuilt"] != null)
                                obj.Years.Year_Built = xnChild.Attributes["YearBuilt"].InnerText;
                            if (xnChild.Attributes["YearUsed"] != null)
                                obj.Years.Year_Used = xnChild.Attributes["YearUsed"].InnerText;
                        ...
                        case "KeyParameters":
                            ...
                            foreach (XmlNode xnChild1 in xnChild.ChildNodes)
                            {
                                if (xnChild1.Name == "KeyParameter")
                                {
                                    ...
                                    if (xnChild1.Attributes["Type"] != null)
                                    {
                                        tmp.Code = xnChild1.Attributes["Type"].InnerText;
                                        tmp.Name = dictTypeParameter.Records
                                            .GetRecByCode(xnChild1.Attributes["Type"].InnerText).Value;
                                    }

                                    ;
                                    if (xnChild1.Attributes["Value"] != null)
                                        tmp.Value = xnChild1.Attributes["Value"].InnerText;
                        ...
                        case "ParentOKS":
                            ...
                            foreach (XmlNode xnChild1 in xnChild.ChildNodes)
                            {
                                ...
                                if (xnChild1.Name == "ExploitationChar")
                                {
                                    ...
                                    if (xnChild1.Attributes["YearBuilt"] != null)
                                        obj.ParentOks.Years.Year_Built = xnChild1.Attributes["YearBuilt"].InnerText;
                                    if (xnChild1.Attributes["YearUsed"] != null)
                                        obj.ParentOks.Years.Year_Used = xnChild1.Attributes["YearUsed"].InnerText;
                                }

                                if (xnChild1.Name == "Floors")
                                {
                                    obj.ParentOks.Floors = new xmlFloors();
                                    if (xnChild1.Attributes["Floors"] != null)
                                        obj.ParentOks.Floors.Floors = xnChild1.Attributes["Floors"].InnerText;
                                    if (xnChild1.Attributes["UndergroundFloors"] != null)
                                        obj.ParentOks.Floors.Underground_Floors =
                                            xnChild1.Attributes["UndergroundFloors"].InnerText;
                                }
                            }
                        ...
                        case "Utilization":
                            ...
                            if (xnChild.Attributes["ByDoc"] != null)
                                obj.Utilization.ByDoc = xnChild.Attributes["ByDoc"].InnerText;
                            if (xnChild.Attributes["PermittedUseText"] != null)
                                obj.Utilization.PermittedUseText = xnChild.Attributes["PermittedUseText"].InnerText;
                            if (xnChild.Attributes["Utilization"] != null)
                            {
                                obj.Utilization.Utilization = new xmlCodeName()
                                {
                                    ...
                                    Name = dictUtilizations.Records.GetValueByCode(xnChild.Attributes["Utilization"]
                                        .InnerText)
                                };
                            }
                            ...
                            if (xnChild.Attributes["LandUse"] != null)
                            {
                                obj.Utilization.LandUse = new xmlCodeName()
                                {
                                    ...
                                    Name = dictAllowedUse.Records.GetValueByCode(
                                        xnChild.Attributes["LandUse"].InnerText)
                                };
                            }
                                ...
                                if (xnChild1.Name == "NaturalObject")
                                {
                                    ...
                                    foreach (XmlNode xnChild2 in xnChild1.ChildNodes)
                                    {
                                        ...
                                        if (xnChild2.Name == "ProtectiveForest")
                                        {
                                            ...
                                            {
                                                Code = xnChild2.InnerText,
                                                Name = dictForestCategoryProtective.Records
                                                    .GetRecByCode(xnChild2.InnerText).Value
                                            };
                                        }
                                            ...
                                            foreach (XmlNode xnChild3 in xnChild2.ChildNodes)
                                            {
                                                if (xnChild3.Name == "ForestEncumbrance")
                                                {
                                                    var forestEncumbrance = new xmlCodeName
                                                    {
                                                        ...
                                                        Name = dictForestEncumbrances.Records
                                                            .GetRecByCode(xnChild3.InnerText).Value
                                                    };
                        ...
                        case "SubParcels":
                            ...
                            foreach (XmlNode xnChild1 in xnChild.ChildNodes)
                            {
                                if (xnChild1.Name == "SubParcel")
                                {
                                    ...
                                    {
                                        subParcel.DateCreated =
                                            DateTime.TryParse(xnChild1.Attributes["DateCreated"].InnerText,
                                                out var date)
                                                ? date
                                                : (DateTime?) null;
                                    }
                            ...
                            foreach (XmlNode xnChild1 in xnChild.ChildNodes)
                            {
                                if (xnChild1.Name == "SubBuilding" || xnChild1.Name == "SubFlat")
                                {
                                    ...
                                    foreach (XmlNode xnChild2 in xnChild1.ChildNodes)
                                    {
                                        ...
                                        if (xnChild2.Name == "Encumbrances")
                                        {
                                            subBuildingFlat.EncumbrancesOks =
                                                GetEncumbrances<xmlEncumbranceOks>(xnChild2);
                                        }
                                    }
                                ...
                                if (xnChild1.Name == "SubConstruction")
                                {
                                    ...
                                    foreach (XmlNode xnChild2 in xnChild1.ChildNodes)
                                    {
                                        if (xnChild2.Name == "KeyParameter")
                                        {
                                            ...
                                            {
                                                subConstruction.KeyParameter.Code =
                                                    xnChild2.Attributes["Type"].InnerText;
                                                subConstruction.KeyParameter.Name = dictTypeParameter.Records
                                                    .GetRecByCode(xnChild2.Attributes["Type"].InnerText).Value;
                                            }

                                            if (xnChild2.Attributes["Value"] != null)
                                                subConstruction.KeyParameter.Value =
                                                    xnChild2.Attributes["Value"].InnerText;
                                        }

                                        if (xnChild2.Name == "Encumbrances")
                                        {
                                            subConstruction.EncumbrancesOks =
                                                GetEncumbrances<xmlEncumbranceOks>(xnChild2);
                                        }
                                    }
                                        ...
                                        if (xnChild4.Name == "ResultsEvent")
                                        {
                                            foreach (XmlNode xnChild5 in xnChild4.ChildNodes)
                                            {
                                                if (xnChild5.Name == "AvailabilityViolations" &&
                                                    !string.IsNullOrEmpty(xnChild5.InnerText))
                                                {
                                                ...
                                                if (xnChild5.Name == "IdentifiedViolations")
                                                {
                                                    ...
                                                    foreach (XmlNode xnChild6 in xnChild5.ChildNodes)
                                                    {
                                                        ...
                                                        {
                                                            encum.IdentifiedViolations.TypeViolations =
                                                                xnChild6.InnerText;
                                                        }

                                                        if (xnChild6.Name == "SignViolations")
                                                        {
                                                            encum.IdentifiedViolations.SignViolations =
                                                                xnChild6.InnerText;
                                                        }
                    ...
                    foreach (XmlNode xnChild2 in xnChild1.ChildNodes)
                    {
                        ...
                        if (xnChild2.Name == "Registration")
                        {
                            ...
                            foreach (XmlNode xnChild3 in xnChild2.ChildNodes)
                            {
                                ...
                                {
                                    encumbrance.Registration.Date =
                                        DateTime.TryParse(xnChild3.InnerText, out var resultDateTime)
                                            ? resultDateTime
                                            : (DateTime?) null;
                                }
                            }
            ...
            GknAllAttributes importedAttributes)
        {
            var objectTypeMapping =
                columnsMapping.First(x => x.AttributeId == RequiredFieldsForExcelMapping.ObjectTypeAttributeId);
            var cadastralNumberMapping = columnsMapping.First(x =>
                x.AttributeId == RequiredFieldsForExcelMapping.CadastralNumberAttributeId);
            var squareMapping =
                columnsMapping.First(x => x.AttributeId == RequiredFieldsForExcelMapping.SquareAttributeId);
            var assessmentDateMapping = columnsMapping.First(x =>
                x.AttributeId == RequiredFieldsForExcelMapping.AssessmentDateAttributeId);
            ...
            mainWorkSheet.Rows.ForEach(row =>
            {
                ...
                catch (Exception ex)
                {
                    Serilog.Log.Error(ex,
                        "Ошибка при обработке {rowIndex} строки excel-файла. Объект '{CadastralNumber}'.",
        ...
        private enTypeObject GetObjectType(ObjectTypeEnumDescriptions descriptions, string typeFromFile)
        {
            ...
            if (string.Equals(descriptions.UncompletedBuilding, typeFromFile,
        ...
        private void LogInfoToReport(ReportInfo reportInfo)
        {
            ...
            _gbuReportService.AddValue(reportInfo.NotProcessedAttributeNames,
                BaseImporter.NotProcessedAttributesColumnIndex, reportRow);
            _gbuReportService.AddValue(reportInfo.TypeConvertingError, BaseImporter.TypeConvertingErrorColumnIndex,
                reportRow);
