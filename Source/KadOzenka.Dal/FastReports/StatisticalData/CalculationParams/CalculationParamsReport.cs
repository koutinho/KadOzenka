...
namespace KadOzenka.Dal.FastReports.StatisticalData.CalculationParams
{
	public class CalculationParamsReport : StatisticalDataReport
	{
		...
		protected override DataSet GetReportData(NameValueCollection query, HashSet<long> objectList = null)
		{
			...
			var factors = ModelFactorsService.GetFactors(model.Id);
		...
		private DataTable GetCoefficientsDataTable(KoAlgoritmType type, string prefix, List<ModelFactorRelation> factors)
		{
			...
			for (var i = 0; i < factors?.Count; i++)
			{
				dataTable.Rows.Add(i + 1,
					factors[i].AttributeName,
					factors[i].GetCoefficient(type));
			}

			return dataTable;
		}

		private List<QuantitativeFactor> GetQuantitativeFactors(List<ModelFactorRelation> factors, long groupId)
		{
			...
			var factorIds = quantitativeFactors.Select(x => x.AttributeId).ToList();
		...
		private List<QualityFactor> GetQualityFactors(List<ModelFactorRelation> factors, long groupId)
		{
			...
			var factorIds = qualityFactors.Select(x => x.AttributeId).ToList();
			if (factorIds.Count <= 0)
