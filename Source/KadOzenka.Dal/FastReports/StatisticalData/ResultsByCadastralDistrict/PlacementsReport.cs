    ...
    public class PlacementsReport : ResultsByCadastralDistrictBaseReport
    {
        ...
        private List<ReportItem> GetOperations(long tourId, List<long> taskIds, InputParameters inputParameters)
        {
            ...
            //units.Where(x => x.ObjectId == objectIdForTesting).ToList().ForEach(unit =>  // для тестирования
            {
                ...
                SetParentPlacementAttributes(tourId, item);
        ...
        private void SetParentPlacementAttributes(long tourId, ReportItem item)
        {
            if (string.IsNullOrWhiteSpace(item.ParentCadastralNumber))
                return;

            var obj = OMMainObject.Where(x => x.CadastralNumber == item.ParentCadastralNumber)
            ...
            var registerIds = new List<long> { groupAttribute.RegisterId };
            var attributeIds = new List<long> { groupAttribute.Id };
            ...
            {
                registerIds.Add(purposeAttribute.RegisterId);
                attributeIds.Add(purposeAttribute.Id);
            }

            var parentAttributes = GbuObjectService.GetAllAttributes(obj.Id, registerIds, attributeIds, DateTime.Now.GetEndOfTheDay());

            var resultForSubGroup = parentAttributes.FirstOrDefault(x => x.AttributeId == groupAttribute.Id);
            item.ParentGroup = resultForSubGroup?.GetValueInString();

            if (purposeAttribute != null)
            {
                var resultForPurpose = parentAttributes.FirstOrDefault(x => x.AttributeId == purposeAttribute.Id);
                item.ParentPurpose = resultForPurpose?.GetValueInString();
            }
        }
