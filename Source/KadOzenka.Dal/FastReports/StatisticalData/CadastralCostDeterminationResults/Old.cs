...
//			//	units.Select(x => x.ObjectId.GetValueOrDefault()).Distinct().Take(2).ToList(),
//			//	new List<long> { new RosreestrRegisterService().RosreestrRegisterId },
//			//	new List<long> { new RosreestrRegisterService().GetAddressAttribute().Id },
//			//	DateTime.Now.GetEndOfTheDay(), isLight: true);
//			var objectIds = new List<long> {7785955};// units.Select(x => x.ObjectId.GetValueOrDefault()).Distinct().Take(2).ToList();
//			var cadastralQuartalGbuAttributes = GbuObjectService.GetAllAttributes(
//				objectIds,
