$.widget("ui.leftSlidePanel", {
    ...
    options: {
        ...
        load_messages_count:     3,                              // количество загружаемых сообщений
        check_conditions_url:    "LeftPanel/CheckConditions",    // метод для проверки состояний контролов
    ...
    _create: function () {
        ...
        self.options.load_content_url = self.options.site_url + self.options.load_content_url;
        self.options.load_messages_url = self.options.site_url + self.options.load_messages_url;
        self.options.check_conditions_url = self.options.site_url + self.options.check_conditions_url;
        ...
        self._setOptions({
            ...
            "load_messages_count": self.options.load_messages_count,
            "check_conditions_url": self.options.check_conditions_url,
		...
		self.element.on('click', '.' + self._class_left_panel_ddl + ' li a:not(.el-disable):not(#customView)', function (e) {
            e.preventDefault();
            var $this = $(this);
        ...
        function CallAction(el) {
            ...
            if (grid && el.data('command')) {
                var rowCount = grid.dataSource.total();
                var rowLimit = el.data('exportLimit');
                var type = 'Xlsx';

                if (el.data('command') === 'ExportToExcel' || el.data('command') === 'ExportToDBF') {
                    if (el.data('command') === 'ExportToExcel')
                        type = 'Xlsx'

                    if (el.data('command') === 'ExportToDBF')
                        type = 'Dbf';

					if (self.options.registerSettings.UseDataReaderMode) {
						showConfirmWindow("Запустить формирование выгрузки в фоновом режиме через менеджер выгрузок?", type);
						return;
					}

					if (rowLimit && rowLimit > 0 && rowCount > rowLimit) {
						showConfirmWindow("Объем выгружаемых данных слишком большой. Запустить формирование выгрузки в фоновом режиме через менеджер выгрузок?", type);
                        return;
                    }
                    else {
                        ExportToFile(self.options.registerSettings.RegistersExportFileUrl, type)
                        return;
                    }
                }
            }
			 ...
			 */
			function showConfirmWindow(message, type) {
				Common.UI.ShowConfirm({
					...
					onSuccess: function (e) {
						...
						$.ajax({
							type: 'POST',
							url: self.options.registerSettings.RegistersExportBackgroundProcessUrl,
							data: { parametersJson: parametersJson, coreExportType: type },
							success: function (result) {
								Common.UI.ShowDialog({
									content: 'Формирование выгрузки запущено. Когда выгрузка будет завершена, Вам будет отправлено уведомление.',
									height: 130,
									icon: 'info',
									showCloseBtn: true
								});
        ...
        function ExportToFile(actionUrl, type) {
            var grid = self.options.registerSettings ?
                $('#Grid-' + self.options.registerSettings.CurrentRegisterId).data('kendoGrid') :
                undefined;
            ...
            if (grid) {
                var parameters = grid.GetDataFunc();
                parameters.CurrentLayoutId = self.options.registerSettings.CurrentLayoutId;
                var url = actionUrl + '?parametersJson=' + fixedEncodeURIComponent(JSON.stringify(parameters)) + '&coreExportType=' + type;
    ...
    _setOption: function (key, value) {
        ...
        var handlers = {
            ...
            "load_messages_count": function () { self.load_messages_count(value); },
            "check_conditions_url": function () { self.check_conditions_url(value); },
        ...
        this._super(key, self.options[key]);
    },
    ...
    bind_events: function () {
        ...
        if (self.options.registerSettings) {
        ...
        function onGridSelectionChange (e) {
            ...
            var objectId = dataItem ? dataItem.id : null;

            self.updateStates(objectId);
        }
    },

    // обновление состояния кнопок левой панели, после изменения выбранной строки грида

    updateStates: function (objectId) {
        var self = this;
        ...
        var requestUrl = $('#Grid-' + self.options.registerSettings.CurrentRegisterId).parent().parent().parent().find('[name=RequestUrl]').val();
    ...
    load_messages_count: function (count) {
        if (count === undefined) {
            return this.options.load_messages_count;
        }

        this.options.load_messages_count = this._valid_integer(count);
    },

    check_conditions_url: function (url) {
        if (url === undefined) {
            return this.options.check_conditions_url;
        }

        this.options.check_conditions_url = url;
    },
    ...
    registerSettings: function (registerSettings) {
        if (window[registerSettings] != undefined && typeof (this.options.registerSettings) != "object")
            this.options.registerSettings = window[registerSettings];
    },
    ...
    _load_content: function (func) {
        ...
        var updateStates = function () {
            if (self.options.context == 'ObjectCard') {
                $.ajax({
                    url: self.options.check_conditions_url,
                    type: 'GET',
                    data: { objectId: Common.GetUrlParameter('ObjId'), registerViewId: Common.GetUrlParameter('RegisterViewId'), uniqueSessionKey: self.options.uniqueSessionKey },
                    dataType: 'json',
                    success: function (states) {
                        if (states) {
                            self.updateButtonsState(states);
                        }
                    }
                })
            }
        ...
        if (!self._contentLoad && self.options.load_content_url) {
            var parameters = { leftPanelId: self.options.panelId, context: self.options.context, uniqueSessionKey: self.options.uniqueSessionKey };
            var urlParameters = {};
            $.map(Common.GetUrlParameters(window.location.href), function (object) {
                urlParameters[object.Name] = object.Value;
            });

            $.ajax({
                url: self.options.load_content_url,
                type: 'GET',
                data: $.extend(parameters, urlParameters),
                success: function (response) {
                    if (response) {
                        self._panel_content.append(response.Value);
                        self._panel_content.find('[data-url]').each(function () {
                            $(this).attr('data-url', $(this).data('url').replace(/\[objectid\]/ig, Common.GetUrlParameter('ObjId')));
                        });
                        ...
                        if (ulList.length > 0) {
                            self.AddMethod("updateButtonsState", function () {
                                if (self.options.context == 'RegisterView')
                                    self.updateStates(-1);
                                updateStates();
                            });
    ...
    _load_list_items: function (item, count) {
        ...
        if (item) {
            ...
            if (url) {
                url = self.options.site_url + url.replace(/^\/+/, '');
    ...
    _list_items_hide: function (el, contentHide) {
        ...
        if (contentHide) {
            el.find('img.lp_arrow').attr('src', this.options.site_url + 'images/arrow_up.png');
            $ul.show(500);

        } else {
            el.find('img.lp_arrow').attr('src', this.options.site_url + 'images/arrow_down.png');
            $ul.hide(500);
        }
    },
