var Common = {
    ...
    ShowWarning: function (message) {
        ...
        notificationPanel.show(value, 'warning');
    },
    ...
    Functions: {
        ...
        FormToObject: function (form, skipHtmlFieldPrefix) {
            ...
            $.each(form.find('input:checkbox:not([data-skip="true"])'), function (index, input) {
                var checkbox = $(input);
                formObject[checkbox.attr('name')] = checkbox.is(':checked');
            });

            //доработка для радио-кнопок
            $.each(form.find('input:radio:not([data-skip="true"])'), function (index, input) {
                var name = $(input).attr('name');

                formObject[name] = form.find('input:radio[name="' + name + '"]:checked').val();
            });
        ...
        ClearForm: function (form) {
            ...
            $.each(form.find('textarea'), function (index, input) {
                $(input).val('');
                $(input).text('');
            });
            ...
            //доработка для для радио-кнопок
            $.each(form.find('input:radio'), function (index, input) {
                $(input).prop('checked', false);
            });
            ...
            $.each(form.find('input[data-role="dropdownlist"]'), function (index, input) {
        ...
        // форматирует число по разрядам. Пример 1000000 -> 1 000 000
        FormatNumber: function (value) {
            if (Common.Functions.IsNumeric(value)) {
    ...
    UI: {
        InputOnlyNumber: function (input) {
        ...
        CreateChooseWindow: function (config) {
            ...
            var window = windowPlacehoder.kendoWindow({
                ...
                refresh: function () {
                    ...
                    var iframeWindo = iframeDomElement.contentWindow;
                    var iframejQ = iframeWindo.$;

                    var fInit = function (iframeDomElement) {
                            ...
                            selectButton
                                ...
                                .on('click', function (e) {
                                    if (config.useCheckedItems) {
                                        ...
                                        if (dataItem == null)
                                            Common.UI.ShowErrorDialog('Не выбрана строка!');
                    ...
                    fInit(iframeDomElement);

                    if (config.childFrame) {
                        config.childFrame(iframejQ('.mainContent'), fInit);
                    }
        ...
        ChooseWindow: function (title, contentUrl, callbackFn, gridName, callbackCancelFn, childFrameCL) {
            return Common.UI.CreateChooseWindow({ title: title, url: contentUrl, callback: callbackFn, closeCallback: callbackCancelFn, gridId: gridName, childFrame: childFrameCL });
        },
        ChooseWindowMultiSelect: function (title, contentUrl, callbackFn, gridName, filter, filterField) {
            filterField = (typeof filterField !== 'undefined') ? filterField : 'id';

            var windowPlacehoder = $('<div></div>');
            ...
            var window = windowPlacehoder.kendoWindow({
                ...
                refresh: function () {
                    ...
                    var gridEl = iframejQuery('#' + gridName);
                    var gridKendoGrid = gridEl.data('kendoGrid');
                    var selectButton = iframejQuery('.k-grid-toolbar .k-button:contains("Выбрать")');

                    if (gridKendoGrid) {
                        gridKendoGrid.bind('dataBinding', function (e) {
                            $.each(e.items, function () {
                                if ($.inArray(this[filterField], filter) > -1) {
                                    this.Selected = true;
                                    this.dirty = true;
                                }
                            });

                            gridKendoGrid.SelectedFilter = filter;
                        });

                        gridKendoGrid.bind('dataBound', function (e) {
                            var grid = this;

                            var $tr = grid.element.find('tr');
        ...
        ShowErrorDialog: function (message) {
            if (message === undefined || message == null || message == '') {
                message = 'Закрыта сессия или потеряна связь с сервером. <br><br>Попробуйте обновить страницу. ';
            }
        ...
        ShowConfirm: function (windowCfg) {
            ...
            Common.UI.ShowDialog({
                ...
                showCloseBtn: false,
                additionalButtons: [{
                    caption: windowCfg.captionYes || 'Да',
                    click: function (e) {
                    ...
                    caption: windowCfg.captionNo || 'Нет',
                    click: windowCfg.onFail
                }]
            });
        },
        ShowInfo: function (windowCfg) {
            if (!windowCfg.onFail) {
                windowCfg.onFail = function () {
                    var window = $(this).closest('[data-role=window]').data('kendoWindow');
                    window.close();
        ...
        ShowWindow: function (title, url, name, onClose, width, height, draggable, resizeble) {
            ...
            var drag = (typeof draggable !== 'undefined') ? draggable : false;
            var resize = (typeof resizeble !== 'undefined') ? resizeble : false;
            ...
            if (onClose && $.isFunction(onClose)) {
                windowCfg.clBackFn = onClose;

                windowCfg.customClose = function (window, e) {
                    onClose(window, e);
                    modalWindow.close();
                    modalWindow.destroy();
                };
            } else {
                windowCfg.customClose = function () {
                    modalWindow.close();
                    modalWindow.destroy();
                };
            }
            ...
            function setWindowContentHeight() {
                ...
                iframe.style.height = "100%";   //newHeight + "px";
        ...
        CloseWindow: function (name, parentWindow, e) {
            if (parentWindow == undefined) {
                if ($("#" + name).length && $("#" + name).data("kendoWindow") != undefined) {
                    if ($("#" + name).data("kendoWindow").options.customClose) {
                        $("#" + name).data("kendoWindow").options.customClose($("#" + name).data("kendoWindow"), e);
                    } else {
                ...
                if (parentWindow.$("#" + name).length && parentWindow.$("#" + name).data("kendoWindow") != undefined) {
                    if (parentWindow.$("#" + name).data("kendoWindow").options.customClose) {
                        parentWindow.$("#" + name).data("kendoWindow").options.customClose(parentWindow.$("#" + name).data("kendoWindow"), e);
                    } else {
...
var KendoExtension = {
    ...
    EditorModal: function (url, title, callback, callbackCancel, hiddenCode = false) {
        var window = $('<div></div>');
    ...
    ToggleEditMode: function (editMode, container) {
        var $elements;
        ...
        if (container)
            $elements = container.find('[editmode="true"]');
        else
            $elements = $('[editmode="true"]');
        ...
        $elements.each(function (i, el) {
            ...
            var comboBox = $(el).data('kendoComboBox');
            if (comboBox) {
                comboBox.readonly(!editMode);
                return;
            }
        });

        if (container == undefined)
            $('input[type=text]' + (editMode ? '[readonly]' : '.read-only')).toggleClass('read-only', editMode);
        else
            container.find('input[type=text]').toggleClass('read-only', !editMode);
    },
    ...
    ExportGridWithTemplatesContent: function (e) {
        ...
        for (var i = 0; i < sheet.columns.length; ++i) {
            ...
            sheet.rows[0].cells[i].wrap = true;
            sheet.rows[0].cells[i].verticalAlign = "center";
...
var waitForElement = function (selector, callback) {
    if ($(window.parent.document).find(selector).length) {
        callback();
    } else {
        setTimeout(function () {
...
/* Добавляем для kendo tooltip событие beforeShow
пример: 
...
$(function () {
    ...
    waitForElement('#main-left-panel #collapse-all', function () {
        var expand = true;
        $(window.parent.document).find('#collapse-all').click(function () {
            if (expand && $('#ls_panelbar').length) {
                $('#ls_panelbar').data('kendoPanelBar').collapse('li');
                expand = false;
                $(this).attr('title', 'Развернуть');
                $(this).find('.left-panel-content-icon').attr('class', 'left-panel-content-icon fas fa-expand-arrows-alt');
            } else if ($('#ls_panelbar').length) {
                $('#ls_panelbar').data('kendoPanelBar').expand('li');
                expand = true;
                $(this).attr('title', 'Свернуть');
                $(this).find('.left-panel-content-icon').attr('class', 'left-panel-content-icon fas fa-compress-arrows-alt');
            }
        });
    });
});