    ...
    if (toolbar) {
        toolbar.add({
            ...
            click: function () {
                ...
                $('iframe#printWindow').remove();

                var htmlContent = $('html').clone();
                var printWindow = $('<iframe id="printWindow" style="visibility: hidden; height: 0px;"></iframe>').appendTo('body')[0].contentWindow;// window.open('', 'PRINT');

                htmlContent.find('script').remove();
                printWindow.document.write(htmlContent.html());
                ...
                setTimeout(function () {
                    ...
                    waitForEl('body', function () {
                        ...
                        // заполнение value для числовых input полей
                        $(printWindow.document).find('input[title]').each(function (i, el) {
                        ...
                        printWindow.addEventListener('afterprint', function () { $('iframe#printWindow').remove(); });
                        printWindow.print();
                    });
