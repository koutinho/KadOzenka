...
{
    /// <summary>
    /// Длительный процесс по отвязке МКД.
    /// </summary>
    public class UnLinkMkdProcess : ILongProcess
    {
        ...
        public void StartProcess(OMProcessType processType, OMQueue processQueue, CancellationToken cancellationToken)
        {
            try
            {
                ...
                processQueue.Message += $"[{DateTime.Now:dd.MM.yyyy HH:mm:ss}]: Получено объектов для отвязки: {dataTable.Rows.Count}\n";
                processQueue.Save();

                List<long> mkds = dataTable?.AsEnumerable()?.Select(x => x["emp_id"].ParseToLong())?.ToList();

                // Обработка.
                if (mkds != null && mkds.Count > 0)
                {
                    foreach (var mkd in mkds)
                    {
                        _buildingService.UnLinkMkd(mkd);
                    }
                }
                else
                {
                    throw new Exception("Не найдены выбранные МКД для отвязки!");
                }
            ...
            {
                ex.AddExtraData($"[{DateTime.Now:dd.MM.yyyy HH:mm:ss}]: Ошибка при отвязке МКД");
