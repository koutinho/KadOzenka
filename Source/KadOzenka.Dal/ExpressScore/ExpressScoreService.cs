...
namespace KadOzenka.Dal.ExpressScore
{
    public class ExpressScoreService
    {
        ...
        {
            var setting = OMSettingsParams.Where(x => x.SegmentType_Code == segmentType).SelectAll()
        ...
        public ParameterDataDto GetEstimateParametersByKn(string kn, int tourId, int attributeId,
            MarketSegment segmentType, int registerId)
        {
            ...
            return unitsIds.Count > 0
                ? ScoreCommonService.GetParameters(unitsIds, attributeId, registerId, qsGroup)
                : null;
        }

        public ParameterDataDto GetEstimateParametersById(int objectId, int attributeId, int registerId,
            MarketSegment? segmentType = null)
        {
        ...
        public List<ComplexCostFactorForCalculateDto> GetCostFactorsForCalculate(string targetKn,
            int? targetMarketObjectId, MarketSegment segment)
        {
            List<ComplexCostFactorForCalculateDto> complexCostFactorsForCalculatePage =
                new List<ComplexCostFactorForCalculateDto>();
            ...
            var koFactors = GetObjectAndCostFactorsByUnitIds(GetSetting(segment), costFactors,
                OMUnit.Where(x => x.CadastralNumber == targetKn).Select(x => x.Id).Execute().Select(x => x.Id)
                    .ToList());
            foreach (var complexCostFactor in complexCostFactors)
            {
                if (IsAnalogAttribute(complexCostFactor.AttributeId.GetValueOrDefault()))
                {
                    ...
                    complexCostFactorsForCalculatePage.Add(
                        new ComplexCostFactorForCalculateDto(complexCostFactor, val ?? complexCostFactor.DefaultValue));
                    continue;
                }

                var koVal = koFactors?.Attributes?.FirstOrDefault(x => x.Id == complexCostFactor.AttributeId)?.Value;
                complexCostFactorsForCalculatePage.Add(
                    new ComplexCostFactorForCalculateDto(complexCostFactor, koVal ?? complexCostFactor.DefaultValue));
            }
        ...
        public QSCondition GetSearchCondition(OMYearConstruction yearRange, OMSquare squareRange, bool useYearBuild,
            bool useSquare, MarketSegment marketSegment, List<DealType> dealType, decimal? lng, decimal? lat)
        {
        ...
        public List<QSJoin> JoinPriceHistory()
        {
            return new List<QSJoin>()
            {
                new QSJoin
                {
                    JoinCondition = new QSConditionSimple(OMPriceHistory.GetColumn(x => x.InitialId),
        ...
        public List<CoordinatesDto> GetNearestCoordinates(Dictionary<long, CoordinatesDto> coordinates,
            Dictionary<long, double> distances, int quantity)
        {
        ...
        /// </returns>
        public List<CoordinatesDto> GetCoordinatesPointAtSelectedDistance(Dictionary<long, CoordinatesDto> coordinates,
            decimal selectedLat, decimal selectedLng, int quantity)
        {
            ...
            foreach (var item in coordinates.Where(x => x.Value.Lng != selectedLng && x.Value.Lat != selectedLat))
            {
                ...
                var d1 = 2 * r *
                         Math.Asin(Math.Sqrt(delLat +
                                             Math.Cos(lat) * Math.Cos(sLat) *
                                             delLng)); // км - расстояние от исходной точки до одного из найденных объектов
        ...
        public string CalculateExpressScore(InputCalculateDto inputParam, out ResultCalculateDto resultCalculate)
        {
            try
            {
                _log.ForContext("inputParam", JsonConvert.SerializeObject(inputParam)).Debug("CalculateExpressScore");
            }
            catch (Exception ex)
            {
                _log.Error(ex, "Ошибка логирования");
            }

            SetRequiredReportParameter(inputParam.TargetObjectId, inputParam.Square, inputParam.Analogs,
                inputParam.Segment, inputParam.Address, inputParam.Kn, inputParam.DealType);
            //"TargetObjectId":16409378,
            //"TargetMarketObjectId":53591344,
            //"Floor":1,
            //"Square":100.0,
            //"ScenarioType":1,
            //"Segment":750,
            //"Address":" г. Москва, ул. Бахрушина, д. 13",
            //"Kn":"77:01:0002012:3801",
            //"DealType":2
            ...
            var squarePerMeterCost =
                CalculateSquarePerMeterCost(calculateSquareCost, out string msg, out List<long> successAnalogIds);
            ...
            msg = SaveSuccessExpressScore(inputParam.TargetObjectId, inputParam.TargetMarketObjectId, summaryCost,
                squarePerMeterCost, out int id, square: inputParam.Square, floor: inputParam.Floor,
                scenarioType: inputParam.ScenarioType,
            ...
            resultCalculate.ReportId = ReportService.GenerateReport(summaryCost, squarePerMeterCost,
                inputParam.DealType, inputParam.ScenarioType);

            var resultAnalogs = inputParam.Analogs.Where(x => successAnalogIds.Contains(x.Id)).Select(x =>
                new AnalogResultDto
                {
                    Id = x.Id,
                    Address = x.Address,
            ...
            resultCalculate.DataToGrid = JsonConvert.SerializeObject(GetDataToGrid(inputParam.Segment, resultAnalogs),
        ...
        {
            dateDict = OMEsReferenceItem.Where(x => x.ReferenceId == exCostFactors.IndexDateDicId).SelectAll().Execute()
                .Select(ScoreCommonService.ReferenceToDate).ToList();
            if (scenarioType != null && scenarioType == ScenarioType.Oks)
                dateNumb = OMEsReferenceItem.Where(x => x.ReferenceId == exCostFactors.LandShareDicId).SelectAll()
                    .Execute().Select(ScoreCommonService.ReferenceToNumber).ToList();
            floorDict = OMEsReferenceItem.Where(x => x.ReferenceId == exCostFactors.FloorDicId).SelectAll().Execute()
            ...
            int expressScoreId, out decimal cost, out decimal squarePerMeterCost, out long reportId)
        {
            ...
            squarePerMeterCost =
                CalculateSquarePerMeterCost(calculateSquareCost, out string msg, out var successAnalogIds);
            ...
            reportId = ReportService.GenerateReport(cost, squarePerMeterCost, inputParam.DealType,
                inputParam.ScenarioType);
            msg = SaveSuccessExpressScore(inputParam.TargetObjectId, inputParam.TargetMarketObjectId, cost,
                squarePerMeterCost, out int id, expressScoreId);
            if (!string.IsNullOrEmpty(msg)) return msg;

            return msg;
        }

        private decimal CalculateSquarePerMeterCost(CalculateSquareCostDto calculateSquareCost, out string msg,
            out List<long> successAnalogIds)
        {
            msg = "";
            ...
            foreach (var analog in calculateSquareCost.Analogs)
            {
                ...
                if (analog.Date != DateTime.MinValue)
                {
                    ...
                    dicText = new KeyValuePair<string, string>(@" Индекс ""Дата предложения""",
                        indexAnalogDate?.Value.ToString() ?? "");
                }
                ...
                var correctText = new KeyValuePair<string, string>(@"Корректировка на дату (Кдата)",
                    Math.Round(kDate, 2).ToString(CultureInfo.InvariantCulture));
                ...
                var analogFloorsCount =
                    GetAnalogFloorsCount(analog.Kn, calculateSquareCost.MarketSegment, exSettingsCostFactors);
                text = new KeyValuePair<string, string>("Этажность",
                    analogFloorsCount == 0 ? string.Empty : analogFloorsCount.ToString());
                dicText = new KeyValuePair<string, string>("Корректировка на долю земельного участка (Кдзу)",
                    value: "1");
                ...
                if (calculateSquareCost.ScenarioType != null && calculateSquareCost.ScenarioType == ScenarioType.Oks)
                {
                    ...
                    {
                        dicText = new KeyValuePair<string, string>("Корректировка на долю земельного участка (Кдзу)",
                            value: Math.Round(coefficient.GetValueOrDefault(), 2).ToString("N"));
                        try
                        {
                ...
                if (calculateSquareCost.DealTypeShort == DealTypeShort.Rent)
                {
                    ...
                    {
                        cost = AddVat(exCostFactors.VatDictionaryId, calculateSquareCost.TargetMarketObjectId, analog,
                            cost, costTargetObjectDataForReport, ref costFactorsDataForReport);
                    }

                    if (exCostFactors.IsOperatingCostsUsedInCalculations.GetValueOrDefault())
                    {
                        cost = AddOperatingCosts(exCostFactors.OperatingCostsCoef,
                            calculateSquareCost.TargetMarketObjectId, analog, cost, costTargetObjectDataForReport,
                            ref costFactorsDataForReport);
                    }
                }
...

                {
                    ...
                    var floorFactor = floor != 0 && floorDict.Count > 0
                        ...
                        : 0;
                    dicText = new KeyValuePair<string, string>(@"Коэффициент ""Этаж расположения""",
                        floorFactor?.ToString("N"));
                    ...
                    var targetObjectFloorFactor = floorDict.Count > 0
                        ? calculateSquareCost.TargetObjectFloor > floorDict.FirstOrDefault().Key
                            ...
                            : floorDict.FirstOrDefault(x => x.Key == calculateSquareCost.TargetObjectFloor)?.Value
                        : 0;
                        ...
                        targetObjectFloorFactor != 0)
                    {
                        ...
                        correctText = new KeyValuePair<string, string>("Корректировка на этаж расположения (K1)",
                            Math.Round(factor.GetValueOrDefault(), 6).ToString("N"));
                ...
                {
                    costFactorsDataForReport.Add(new Tuple<string, string>(
                        "Вид сделки (Предложение-продажа/Сделка купли-продажи)", analog.DealType.GetEnumDescription()));
                    costTargetObjectDataForReport.Add("");
                }
                ...
                {
                    costFactorsDataForReport.Add(new Tuple<string, string>(
                        "Вид сделки (Предложение-аренда/Сделка-аренда)", analog.DealType.GetEnumDescription()));
                    costTargetObjectDataForReport.Add("");
                }
                ...
                foreach (var simple in exCostFactors.SimpleCostFactors)
                {
                    ...
                    if (simple.Coefficient != null)
                    {
                        ...
                        {
                            GenerateOverflowException(e, analog.Kn, $"Статичный коэффициент: {simple.Name}",
                                simple.Coefficient);
                        }
                    }
                ...
                var idAnalog = OMCoreObject.Where(x => x.CadastralNumber == calculateSquareCost.Kn)
                    .ExecuteFirstOrDefault()?.Id;


                int countRow = costFactorsDataForReport.Count +
                               GetCountReportRowComplexFactors(exCostFactors.ComplexCostFactors);
                ...
                foreach (var complex in exCostFactors.ComplexCostFactors)
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            {
                                var esTargetObjectValue = OMTargetObjectValue
                                    .Where(x => x.UnitId == calculateSquareCost.TargetObjectId).SelectAll()
                                    .ExecuteFirstOrDefault();

                                var targetAttributeValue =
                                    esTargetObjectValue.AttributeValue.DeserializeFromXml<List<AttributeValueDto>>();

                                var attributeValue = targetAttributeValue
                                    .FirstOrDefault(x => x.Id == complex.AttributeId).Value;
                                targetObjectFactor = new ParameterDataDto(new PureParameterDataDto
                                {
                                    Id = calculateSquareCost.TargetObjectId,
                    ...
                    string valueToComplexName = analogFactor.NumberValue != 0
                        ? analogFactor.NumberValue.ToString("N")
                        : analogFactor.Value?.ToString();

                    AddReportDictValue(ref costFactorsDataForReport,
                        new KeyValuePair<string, string>(complex.Name, valueToComplexName));
                    costTargetObjectDataForReport.Add(targetObjectFactor?.Value != null
                    ...
                    switch (analogFactor.Type)
                    {
                        case ParameterType.String:
                        {
                            if (complex.DictionaryId != null && complex.DictionaryId != 0)
                            {
                                ...
                                if (analogC == 0 || targetObjectC == 0)
                                {
                                    ...
                                    AddReportDictValue(ref costFactorsDataForReport,
                                        new KeyValuePair<string, string>(
                                            "Корректировка " + @"""" + complex.Name + @"""" +
                                            $" K({amountSuccessComplexFactors})", "1"));
                                    costTargetObjectDataForReport.Add("");
                                    break;
                                }
                                ...
                                AddReportDictValue(ref costFactorsDataForReport,
                                    new KeyValuePair<string, string>(
                                        "Корректировка " + @"""" + complex.Name + @"""" +
                                        $" K({amountSuccessComplexFactors})", coeff.ToString("N")));
                        ...
                        {
                            _log.Warning(
                                "ЭО. Дефолная обработка оценочного фактора, т.к не был найден фактор в Ко части или в аналогах");
                            if (complex.DictionaryId != null && complex.DictionaryId != 0)
                            {
                                AddReportDictValue(ref costFactorsDataForReport,
                                    new KeyValuePair<string, string>("Метка " + @"""" + complex.Name + @"""", ""));
                ...
                {
                    AddReportDictValue(ref costFactorsDataForReport,
                        new KeyValuePair<string, string>("Скорректированная стоимость руб/кв.м",
                            Math.Round(cost, 2).ToString("N")));
                }
                ...
                if (calculateSquareCost.DealTypeShort == DealTypeShort.Rent)
                {
                    AddReportDictValue(ref costFactorsDataForReport,
                        new KeyValuePair<string, string>(
                            "Скорректированная арендная ставка объектов-аналогов, руб/кв.м/год",
                            (Math.Round(cost, 2) * 12).ToString("N")));
                }

                costTargetObjectDataForReport.Add("");

                WriteByColumnToComplexMatrix(costFactorsDataForReport, costTargetObjectDataForReport,
                    needWriteTargetObjectDataToReport);
            ...
            List<string> costTargetObjectDataForReport, ref List<Tuple<string, string>> costFactorsDataForReport)
        {
            ...
            AddReportDictValue(ref costFactorsDataForReport,
                new KeyValuePair<string, string>("Наличие НДС", analog.Vat?.GetEnumDescription()));
            AddReportDictValue(ref costFactorsDataForReport,
                new KeyValuePair<string, string>("Метка (С НДС/Без НДС)", analogLabel));
            AddReportDictValue(ref costFactorsDataForReport,
                new KeyValuePair<string, string>("Корректировка на НДС", analogCorrection?.ToString()));
        ...
        private decimal AddOperatingCosts(decimal? operatingCostsCoef, long? targetMarketObjectId, AnalogDto analog,
            decimal cost,
            List<string> costTargetObjectDataForReport, ref List<Tuple<string, string>> costFactorsDataForReport)
        {
            ...
            AddReportDictValue(ref costFactorsDataForReport, new KeyValuePair<string, string>(
                "Наличие операционных расходов",
                analog.IsOperatingCostsIncluded.GetValueOrDefault() ? "Включены" : "Не включены"));
            AddReportDictValue(ref costFactorsDataForReport,
                new KeyValuePair<string, string>("Корректировка на операционные расходы",
                    analogCorrection?.ToString()));

            bool? targetMarketObjectIsOperatingCostsIncluded = null;
            if (targetMarketObjectId != null)
            {
                targetMarketObjectIsOperatingCostsIncluded = OMCoreObject.Where(x => x.Id == targetMarketObjectId)
                    .Select(x => x.IsOperatingCostsIncluded)
                    .ExecuteFirstOrDefault()?.IsOperatingCostsIncluded;
            }

            costTargetObjectDataForReport.Add(targetMarketObjectIsOperatingCostsIncluded.GetValueOrDefault()
                ? "Включены"
                : "Не включены");
            costTargetObjectDataForReport.Add(string.Empty);
        ...
        private void GenerateOverflowException(Exception e, string cadastralNumber, string multipliedName,
            object multipliedValue)
        {
            var message =
                $"Во время обработки аналога '{cadastralNumber}' возникло переполнение при умножении Стоимости на '{multipliedName} ({multipliedValue})'.";
        ...
        /// <param name="unitIds"></param>
        /// <returns></returns>
        private TargetObjectDto GetObjectAndCostFactorsByUnitIds(OMSettingsParams setting, CostFactorsDto costFactor,
            List<long> unitIds)
        {
            ...
            try
            {
                ...
                var query = ScoreCommonService.GetQsQuery((int) setting.Registerid,
                    (int) tourRegisterPrimaryKeyId.GetValueOrDefault(), unitIds);

                query.AddColumn((long) costFactor.YearBuildId.GetValueOrDefault(),
                    ((int) costFactor.YearBuildId.GetValueOrDefault()).ToString());
                foreach (var factor in costFactor.ComplexCostFactors)
                {
                    if (!IsAnalogAttribute(factor.AttributeId.GetValueOrDefault()))
                        query.AddColumn(factor.AttributeId.GetValueOrDefault(),
                            factor.AttributeId.GetValueOrDefault().ToString());
                }
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    foreach (var factor in costFactor.ComplexCostFactors.Where(x =>
                        !IsAnalogAttribute(x.AttributeId.GetValueOrDefault())))
                    {
        ...
        private List<AttributePure> GetAnalogCostFactors(CostFactorsDto costFactors, string kn = null, int? id = null)
        {
            ...
            try
            {
                ...
                {
                    _log.ForContext("costFactors===>", costFactors)
                        .Error("ЭО.Получение оценочных факторов для аналогов");
                ...
                {
                    if (IsAnalogAttribute(complexCostFactor.AttributeId.GetValueOrDefault()))
                        coreObject.AddColumn(complexCostFactor.AttributeId.GetValueOrDefault(),
                            complexCostFactor.AttributeId.GetValueOrDefault().ToString());
                }
                ...
                {
                    foreach (var factor in costFactors.ComplexCostFactors.Where(x =>
                        IsAnalogAttribute(x.AttributeId.GetValueOrDefault())))
                    {
                ...
                if (esTargetObjectValue != null)
                {
                    ...
                    if (esTargetObjectAttributeValue != null && esTargetObjectAttributeValue.Count > 0)
                    {
                        var existedAttributeValue =
                            esTargetObjectAttributeValue.Where(x => attributeValues.Keys.ToList().Contains(x.Id));
        ...
        public DataToGrid GetDataToGrid(MarketSegment marketSegment, List<AnalogResultDto> analogResults)
        {
            ...
            {
                DataField = OMCoreObject.GetColumnAttributeId(x => x.CadastralNumber).ToString(),
                Text = "Кадастровый номер", Width = 200
            });
            res.Headers.Add(new Header
            {
                DataField = OMCoreObject.GetColumnAttributeId(x => x.Address).ToString(),
            ...
            if (costFactors != null)
            {
                ...
                {
                    res.Headers.Add(new Header
                        {DataField = complexFactor.AttributeId.ToString(), Text = complexFactor.Name});
                }
                ...
                foreach (var analogResult in analogResults)
                {
                    ...
                    analogCostFactors.Add(new AttributePure
                    {
                        Id = (int) OMCoreObject.GetColumnAttributeId(x => x.CadastralNumber), Value = analogResult.Kn
                    });
                    analogCostFactors.Add(new AttributePure
                        {Id = (int) OMCoreObject.GetColumnAttributeId(x => x.Address), Value = analogResult.Address});
                    analogCostFactors.Add(new AttributePure
                    {
                        Id = (int) OMCoreObject.GetColumnAttributeId(x => x.Price),
                        Value = analogResult.Price.ToString()
                    });
                    analogCostFactors.AddRange(GetAnalogCostFactors(costFactors, id: (int) analogResult.Id));

                    row.AddRange(analogCostFactors.Select(x => new Cell
                        {Key = x.Id.ToString(), CommonValue = x.Value}));

                    var unitsIds =
                        ScoreCommonService.GetUnitsIdsByCadastralNumber(analogResult.Kn, (int) setting.TourId);
                    var obj = unitsIds.Count > 0
                        ? GetObjectAndCostFactorsByUnitIds(setting, costFactors, unitsIds)
                        : null;

                    if (obj != null)
                    {
                        row.AddRange(
                            obj.Attributes.Select(x => new Cell {Key = x.Id.ToString(), CommonValue = x.Value}));
                    }
        ...
        public string SaveSuccessExpressScore(int targetObjectId, long? targetMarketObjectId, decimal summaryCost,
            decimal costSquareMeter, out int id, int? expressScoreId = null,
            decimal? square = null, int? floor = null, ScenarioType? scenarioType = null,
            MarketSegment? segmentType = null, DealType? dealType = null, string address = null)
        {
            id = 0;
            try
            {
                var kn = OMUnit.Where(x => x.Id == targetObjectId).Select(x => x.CadastralNumber)
                    .ExecuteFirstOrDefault()?.CadastralNumber;
                ...
                id = expressScoreId == null
                    ? AddExpressScore(kn, summaryCost, costSquareMeter, square.Value, floor.Value, targetObjectId,
                        targetMarketObjectId, scenarioType.GetValueOrDefault(), segmentType.GetValueOrDefault(),
                        dealType.GetValueOrDefault(), address)
        ...
        private void SetRequiredReportParameter(int targetObjectId, decimal targetSquare, List<AnalogDto> analogs,
            MarketSegment marketSegment, string address, string kn, DealTypeShort dealType)
        {
            try
            {
                ...
                var targetObjectYear = GetEstimateParametersById(targetObjectId,
                    (int) costFactor.YearBuildId.GetValueOrDefault(), (int) exSettingsCostFactors.Registerid,
                    marketSegment);
                ...
                ReportService.AddValueRequiredParam(
                    targetObjectYear?.NumberValue.ToString(CultureInfo.InvariantCulture));
                ...
                ReportService.AddNameCharacteristicRequiredParam(dealType == DealTypeShort.Rent
                    ? "Стоимость объекта-аналога, руб/год"
                    : "Стоимость объекта-аналога, руб");
                ReportService.AddValueRequiredParam("-");

                ReportService.AddNameCharacteristicRequiredParam(dealType == DealTypeShort.Rent
                    ? "Стоимость объекта-аналога, руб/кв.м/год"
                    : "Стоимость объекта-аналога, руб/кв.м");
                ReportService.AddValueRequiredParam("-");
                ...
                foreach (AnalogDto analog in analogs)
                {
                    ...
                    if (dealType == DealTypeShort.Rent)
                    {
                        ...
                        ReportService.AddValueRequiredParam(Math.Round(analog.Price * 12 / analog.Square, 2)
                            .ToString("N"));
                    }
                    else
                    {
            ...
            {
                _log.Error(ex, "Ошибка SetRequiredReportParameter ");
            }
        } 
