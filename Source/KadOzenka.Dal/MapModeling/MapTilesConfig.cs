...
namespace KadOzenka.Dal.MapModeling
{
	public class MapTilesConfig
	{
		...
		public static string GetMarketHeatMapTilesFolder(int currentZoom)
		{
			if (!Directory.Exists(
				$"{Current.MarketMapHeatMapLayerFolder}\\user_{SRDSession.GetCurrentUserId()}\\{currentZoom}"))
			{
				Directory.CreateDirectory($"{Current.MarketMapHeatMapLayerFolder}\\user_{SRDSession.GetCurrentUserId()}\\{currentZoom}");
			}
		...
		public static string GetMarketHeatMapTileFileName(int x, int y, int currentZoom)
		{
			return
				$"{Current.MarketMapHeatMapLayerFolder}\\user_{SRDSession.GetCurrentUserId()}\\{currentZoom}\\{x}_{y}.png";
		}
