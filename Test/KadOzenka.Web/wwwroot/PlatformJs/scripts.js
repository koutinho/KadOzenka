                ...
                refresh: function () {
                    ...
                    if (gridKendoGrid) {
                        gridKendoGrid.bind('dataBound', function (e) {
                            ...
                            grid.element.find('tbody tr[data-uid]').dblclick(function (e) {
                                e.preventDefault();
                                iframejQuery('.k-grid-toolbar .k-button:contains("Выбрать")').click();
                            });

                            $tr.each(function () {
                                var $el = $(this);
                                var $dataItem = grid.dataItem($el);
                                if (filter && filterField && $dataItem) {
                                    if ($dataItem.hasOwnProperty(filterField)) {
                                        if ($.inArray($dataItem[filterField].toString(), filter) > -1) {
                                            var cb = $el.find("Input[type=checkbox]").prop("disabled", true);
                                            cb.next("label").css('cursor', 'not-allowed');
                                        }
                                    }
                                }
                            });
                        ...
                        iframejQuery('.k-grid-toolbar .k-button:contains("Выбрать")').off('click');
                        iframejQuery('.k-grid-toolbar .k-button:contains("Выбрать")').on('click', function (e) {
                            e.preventDefault();
                            var grid = gridEl.data('kendoGrid');
                            var data = [];

                            var $tr = grid.content.find('tr').has('input:checked');

                            if (!$tr.length) {
                                Common.UI.ShowDialog({
                                    title: 'Ошибка',
                                    content: 'Не выбрано ни одной строки!',
                                    icon: 'error',
                                    showCloseBtn: true
                                });
                            }
